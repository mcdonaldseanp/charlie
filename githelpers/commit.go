...
func NewCommit(maybe_message string) error {
	...
	if len(maybe_message) > 0 {
		arr = localexec.ExecAsShell(nil, "git", "commit", "-m", maybe_message)
	} else {
		arr = localexec.ExecAsShell(nil, "git", "commit")
	}
	return arr
}
...
func AddCommit(no_edit bool) error {
	...
	if no_edit {
		...
		arr := localexec.ExecAsShell(nil, "git", "commit", "--amend", "--no-edit")
		return arr
	} else {
		arr := error(localexec.ExecAsShell(nil, "git", "commit", "--amend"))
		return arr
	}
}

func addAllCLI() error {
	return localexec.ExecAsShell(nil, "git", "add", "--all")
}
